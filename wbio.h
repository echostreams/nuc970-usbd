...
#define inpb(port)            0//(*((UINT8 volatile *) (port)))
#define outphw(port,value)    //(*((UINT16 volatile *) (port))=value)
#define inphw(port)           0//(*((UINT16 volatile *) (port)))
#define outpw(port,value)     //(*((UINT32 volatile *) (port))=value)
#define inpw(port)            0//(*((UINT32 volatile *) (port)))

#define readb(addr)           0//(*(UINT8 volatile *)(addr))
#define writeb(addr,x)        //((*(UINT8 volatile *)(addr)) = (UINT8 volatile)x)
#define readhw(addr)          0//(*(UINT16 volatile *)(addr))
#define writehw(addr,x)       //((*(UINT16 volatile *)(addr)) = (UINT16 volatile)x)
#define readw(addr)           0//(*(UINT32 volatile *)(addr))
